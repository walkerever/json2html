...
def json2html_main():
    ...
    parser.add_argument( "-a", "--table_attributes", dest="attributes", default="border-collapse:collapse;", help="table attributes",)
    parser.add_argument( "-s", "--sorted", dest="sorted", action="store_true", default=False, help="try best to make sure output is sorted. default by key.",)
    parser.add_argument( "-C", "--colors", dest="colors", help="background colors to be used.")
    parser.add_argument( "-k", "--keywords_order", dest="keyorder", help="use this ordered keywords to help sort dict.",)
    parser.add_argument( "-I", "--keys-included", dest="keys_included", action="append", help="include these TOP level keys only",)
    parser.add_argument( "-E", "--keys-excluded", dest="keys_excluded", action="append", help="exclude these keys. eg. k1 or k2.k3",)
    parser.add_argument( "-v", "--dictsortbyvalue", dest="dtbyval", action="store_true", default=False, help="with -s, sort dict by value.",)
    parser.add_argument( "-r", "--recursive", dest="recursive", action="store_true", default=False, help="if str value is JSON, treat it as JSON object ",)
    parser.add_argument( "-Z", "--collapseandexpand", dest="collapseandexpand", action="store_true", default=False, help="make the HTML table able to expand/collapse. this may make it looks less formal.",
    )
